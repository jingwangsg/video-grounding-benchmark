...
class CTRL_Model(object):
    ...
    def cross_modal_comb(self, visual_feat, sentence_embed, batch_size):
        vv_feature = tf.reshape(
            tf.tile(visual_feat, [batch_size, 1]), [batch_size, batch_size, self.semantic_size]
        )
        ss_feature = tf.reshape(
            tf.tile(sentence_embed, [1, batch_size]), [batch_size, batch_size, self.semantic_size]
        )
        print(ss_feature.get_shape().as_list())
        print(vv_feature.get_shape().as_list())
        print(batch_size, self.semantic_size)
        concat_feature = tf.reshape(
            tf.concat(axis=2, values=[vv_feature, ss_feature]),
        ...
        comb_feature = tf.reshape(
            tf.concat(axis=2, values=[mul_feature, add_feature, concat_feature]),
    ...
    def visual_semantic_infer(
        self, visual_feature_train, sentence_embed_train, visual_feature_test, sentence_embed_test
    ):
        name = "CTRL_Model"
        with tf.variable_scope(name):
            print("Building training network...............................\n")
            transformed_clip_train = fc(
                "v2s_lt", visual_feature_train, output_dim=self.semantic_size
            )
            transformed_clip_train_norm = tf.nn.l2_normalize(transformed_clip_train, dim=1)
            transformed_sentence_train = fc(
                "s2s_lt", sentence_embed_train, output_dim=self.semantic_size
            )
            ...
            cross_modal_vec_train = self.cross_modal_comb(
                transformed_clip_train_norm, transformed_sentence_train_norm, self.batch_size
            )
            sim_score_mat_train = vs_multilayer.vs_multilayer(
                cross_modal_vec_train, "vs_multilayer_lt", middle_layer_dim=1000
            )
            ...
            transformed_sentence_test = fc(
                "s2s_lt", sentence_embed_test, output_dim=self.semantic_size
            )
            ...
            cross_modal_vec_test = self.cross_modal_comb(
                transformed_clip_test_norm, transformed_sentence_test_norm, self.test_batch_size
            )
            sim_score_mat_test = vs_multilayer.vs_multilayer(
                cross_modal_vec_test, "vs_multilayer_lt", reuse=True, middle_layer_dim=1000
            )
    ...
    def init_placeholder(self):
        ...
        sentence_ph_test = tf.placeholder(
            tf.float32, shape=(self.test_batch_size, self.sentence_embedding_size)
        )

        return (
            visual_featmap_ph_train,
            sentence_ph_train,
            offset_ph,
            visual_featmap_ph_test,
            sentence_ph_test,
        )
    ...
    def construct_model(self):
        ...
        (
            self.visual_featmap_ph_train,
            self.sentence_ph_train,
            self.offset_ph,
            self.visual_featmap_ph_test,
            self.sentence_ph_test,
