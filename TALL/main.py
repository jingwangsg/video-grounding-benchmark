...
def compute_IoU_recall_top_n_forreg(
    top_n, iou_thresh, sentence_image_mat, sentence_image_reg_mat, sclips, iclips
):
    correct_num = 0.0
...
def do_eval_slidingclips(sess, vs_eval_op, model, movie_length_info, iter_step, test_result_output):
    ...
    for movie_name in model.test_set.movie_names:
        ...
        movie_clip_featmaps, movie_clip_sentences = model.test_set.load_movie_slidingclip(
            movie_name, 16
        )
        ...
        for k in range(len(IoU_thresh)):
            IoU = IoU_thresh[k]
            correct_num_10 = compute_IoU_recall_top_n_forreg(
                10, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips
            )
            correct_num_5 = compute_IoU_recall_top_n_forreg(
                5, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips
            )
            correct_num_1 = compute_IoU_recall_top_n_forreg(
                1, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips
            )
            print(
                movie_name
                + " IoU="
                + str(IoU)
                + ", R@10: "
                + str(correct_num_10 / len(sclips))
                + "; IoU="
                + str(IoU)
                + ", R@5: "
                + str(correct_num_5 / len(sclips))
                + "; IoU="
                + str(IoU)
                + ", R@1: "
    ...
    for k in range(len(IoU_thresh)):
        print(
            ...
            + str(IoU_thresh[k])
            + ", R@10: "
            + str(all_correct_num_10[k] / all_retrievd)
            + "; IoU="
            + str(IoU_thresh[k])
            + ", R@5: "
            + str(all_correct_num_5[k] / all_retrievd)
            + "; IoU="
            + str(IoU_thresh[k])
            + ", R@1: "
            + str(all_correct_num_1[k] / all_retrievd)
        )
        test_result_output.write(
            "Step "
            + str(iter_step)
            + ": IoU="
            + str(IoU_thresh[k])
...
def run_training():
    ...
    test_feature_dir = "./.data-bin/Interval128_256_overlap0.8_c3d_fc6/"
    train_feature_dir = "./.data-bin/Interval64_128_256_512_overlap0.8_c3d_fc6/"

    model = ctrl_model.CTRL_Model(
        batch_size, train_csv_path, test_csv_path, test_feature_dir, train_feature_dir
    )
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(max_steps):
            ...
            _, loss_value, offset_pred_v, loss_reg_v = sess.run(
                [vs_train_op, loss_align_reg, offset_pred, loss_reg], feed_dict=feed_dict
            )
            ...
            if (step + 1) % 2000 == 0:
                ...
                do_eval_slidingclips(
                    sess, vs_eval_op, model, movie_length_info, step + 1, test_result_output
                )
