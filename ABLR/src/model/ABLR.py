...
def get_word_embedding(
    word_embedding_path, wordtoix_path, ixtoword_path, extracted_word_fts_init_path
):
...
def parse_args():
    ...
    parser.add_argument("--net", dest="model", help="model to test", default=None, type=str)
    parser.add_argument("--task", dest="task", help="train or test", default="train", type=str)
    parser.add_argument(
        "--layer",
        dest="regress_layer_num",
...
def smooth_l1_loss(predict_location, video_location):
    ...
    smooth_l1norm = tf.reduce_sum(
        tf.where(abs_diff_lt_1, 0.5 * tf.square(abs_diff), abs_diff - 0.5), 1
    )
...
class ABLR:
    def __init__(
        ...
        if self.dim_image == 4096:
            self.embed_video_w = tf.Variable(
                tf.random_uniform([self.dim_image, self.dim_image_encode], -0.1, 0.1),
                name="embed_video_w",
            )
            self.embed_video_b = tf.Variable(
                tf.zeros([self.dim_image_encode]), name="embed_video_b"
            )

        # LSTM for encode video first
        self.lstm_video = LSTMCell(num_units=dim_hidden, use_peepholes=True)
        self.lstm_video_dropout = DropoutWrapper(
            self.lstm_video, output_keep_prob=1 - self.drop_out_rate
        )

        # Reverse LSTM for encode video
        self.reverse_lstm_video = LSTMCell(num_units=dim_hidden, use_peepholes=True)
        self.reverse_lstm_video_dropout = DropoutWrapper(
            self.reverse_lstm_video, output_keep_prob=1 - self.drop_out_rate
        )

        # LSTM for encode sentence first
        self.lstm_sentence = LSTMCell(num_units=dim_hidden, use_peepholes=True)
        self.lstm_sentence_dropout = DropoutWrapper(
            self.lstm_sentence, output_keep_prob=1 - self.drop_out_rate
        )

        # Reverse LSTM for encode sentence
        self.reverse_lstm_sentence = LSTMCell(num_units=dim_hidden, use_peepholes=True)
        self.reverse_lstm_sentence_dropout = DropoutWrapper(
            self.reverse_lstm_sentence, output_keep_prob=1 - self.drop_out_rate
        )

        self.embed_att_w = tf.Variable(
            tf.random_uniform([dim_hidden, 1], -0.1, 0.1), name="embed_att_w"
        )
        self.embed_att_Wa = tf.Variable(
            tf.random_uniform([dim_hidden, dim_hidden], -0.1, 0.1), name="embed_att_Wa"
        )
        self.embed_att_Ua = tf.Variable(
            tf.random_uniform([dim_hidden, dim_hidden], -0.1, 0.1), name="embed_att_Ua"
        )
        self.embed_att_ba = tf.Variable(tf.zeros([dim_hidden]), name="embed_att_ba")

        self.embed_att_w_q = tf.Variable(
            tf.random_uniform([dim_hidden, 1], -0.1, 0.1), name="embed_att_w_q"
        )
        self.embed_att_W_qa = tf.Variable(
            tf.random_uniform([dim_hidden, dim_hidden], -0.1, 0.1), name="embed_att_W_qa"
        )
        self.embed_att_U_qa = tf.Variable(
            tf.random_uniform([dim_hidden, dim_hidden], -0.1, 0.1), name="embed_att_U_qa"
        )
        self.embed_att_b_qa = tf.Variable(tf.zeros([dim_hidden]), name="embed_att_b_qa")

        self.w_fuse_video = tf.Variable(
            tf.random_uniform([2 * dim_hidden, dim_hidden], -0.1, 0.1), name="w_fuse_video"
        )
        self.b_fuse_video = tf.Variable(tf.zeros([dim_hidden]), name="b_fuse_video")

        self.w_fuse_sentence = tf.Variable(
            tf.random_uniform([2 * dim_hidden, dim_hidden], -0.1, 0.1), name="w_fuse_sentence"
        )
        self.b_fuse_sentence = tf.Variable(tf.zeros([dim_hidden]), name="b_fuse_sentence")

        if self.model_mode == "AF":
            self.w_fuse = tf.Variable(
                tf.random_uniform([2 * dim_hidden, dim_hidden], -0.1, 0.1), name="w_fuse"
            )
            self.b_fuse = tf.Variable(tf.zeros([dim_hidden]), name="b_fuse")
            if regress_layer_num == 1:
                self.regress_w = tf.Variable(
                    tf.random_uniform([dim_hidden, 2], -0.1, 0.1), name="regress_w"
                )
                self.regress_b = tf.Variable(tf.zeros([2]), name="regress_b")
            else:
                self.regress_w = tf.Variable(
                    tf.random_uniform([dim_hidden, dim_hidden_regress], -0.1, 0.1), name="regress_w"
                )
                self.regress_b = tf.Variable(tf.zeros([dim_hidden_regress]), name="regress_b")
                self.regress_w_2 = tf.Variable(
                    tf.random_uniform([dim_hidden_regress, 2], -0.1, 0.1), name="regress_w_2"
                )
                self.regress_b_2 = tf.Variable(tf.zeros([2]), name="regress_b_2")
        elif self.model_mode == "AW":
            if regress_layer_num == 1:
                self.regress_w = tf.Variable(
                    tf.random_uniform([n_frame_step, 2], -0.1, 0.1), name="regress_w"
                )
                self.regress_b = tf.Variable(tf.zeros([2]), name="regress_b")
            else:
                self.regress_w = tf.Variable(
                    tf.random_uniform([n_frame_step, dim_hidden_regress], -0.1, 0.1),
                    name="regress_w",
                )
                self.regress_b = tf.Variable(tf.zeros([dim_hidden_regress]), name="regress_b")
                self.regress_w_2 = tf.Variable(
    ...
    def build_model(
        self,
    ):  # caption step: means the sentence length, # n_frame_step: means the video segment nums
        ...
        with tf.variable_scope(tf.get_variable_scope()) as model_scope:
            ...
            video = tf.placeholder(
                tf.float32, [self.batch_size, self.n_frame_step, self.dim_image]
            )  # b x n x d
            ...
            caption = tf.placeholder(
                tf.int32, [self.batch_size, n_caption_step]
            )  # b x n_caption_step
            caption_mask = tf.placeholder(
                tf.float32, [self.batch_size, n_caption_step]
            )  # b x n_caption_step
            caption_emb_mask = tf.placeholder(
                tf.float32, [n_caption_step, self.batch_size, dim_hidden]
            )  # indicate where do caption end
            ...
            if self.dim_image == 4096:
                ...
                video_emb = tf.nn.xw_plus_b(
                    video_flat, self.embed_video_w, self.embed_video_b
                )  # (b x n) x h
                video_emb = tf.reshape(
                    video_emb, [self.batch_size, self.n_frame_step, self.dim_image_encode]
                )  # b x n x h
            else:
                video_emb = video
            ...
            # ordinal sequence
            state1_video = (
                tf.zeros([self.batch_size, self.lstm_video.state_size[0]]),
                tf.zeros([self.batch_size, self.lstm_video.state_size[1]]),
            )
            output_video_list = []
            for i in range(n_frame_step):
                current_embed_video = video_emb[:, i, :]  # bxnxd
                with tf.variable_scope("LSTM_video"):
                    output_video, state1_video = self.lstm_video_dropout(
                        current_embed_video, state1_video
                    )
                    if i > 0:
            ...
            # reverse sequence
            reverse_state1_video = (
                tf.zeros([self.batch_size, self.reverse_lstm_video.state_size[0]]),
                tf.zeros([self.batch_size, self.reverse_lstm_video.state_size[1]]),
            )
            reverse_output_video_list = []
            for i in range(n_frame_step):
                current_embed_video = video_emb[:, n_frame_step - i - 1, :]  # bxnxd
                with tf.variable_scope("Reverse_LSTM_video"):
                    reverse_output_video, reverse_state1_video = self.reverse_lstm_video_dropout(
                        current_embed_video, reverse_state1_video
                    )
                    if i > 0:
                        tf.get_variable_scope().reuse_variables()
            ...
            for i in range(n_frame_step):
                new_reverse_output_video_list.append(
                    reverse_output_video_list[n_frame_step - i - 1]
                )
            reverse_sequence_video_emb = tf.stack(new_reverse_output_video_list)  # n x b x h

            sequence_video_emb = tf.concat(
                [ordinal_sequence_video_emb, reverse_sequence_video_emb], 2
            )  # nxbx2h
            sequence_video_emb_flat = tf.reshape(
                sequence_video_emb, [-1, self.dim_hidden * 2]
            )  # (nxb,2h)
            sequence_video_combine = tf.nn.relu(
                tf.nn.xw_plus_b(sequence_video_emb_flat, self.w_fuse_video, self.b_fuse_video)
            )
            sequence_video_combine = tf.reshape(
                sequence_video_combine, [n_frame_step, self.batch_size, self.dim_hidden]
            )
            ...
            # ordinal sequence
            state1_sentence = (
                tf.zeros([self.batch_size, self.lstm_sentence.state_size[0]]),
                tf.zeros([self.batch_size, self.lstm_sentence.state_size[1]]),
            )
            output1_sentence_list = []
            for i in range(n_caption_step):
                with tf.device("/cpu:0"):
                    current_embed_sentence = tf.nn.embedding_lookup(
                        self.Wemb, caption[:, i]
                    )  # caption b x n_caption_step
                with tf.variable_scope("LSTM_sentence"):
                    output1_sentence, state1_sentence = self.lstm_sentence_dropout(
                        current_embed_sentence, state1_sentence
                    )  # b x h
                    if i > 0:
                        tf.get_variable_scope().reuse_variables()
            ...
            # reverse sequence
            reverse_state1_sentence = (
                tf.zeros([self.batch_size, self.reverse_lstm_sentence.state_size[0]]),
                tf.zeros([self.batch_size, self.reverse_lstm_sentence.state_size[1]]),
            )
            reverse_output1_sentence_list = []
            for i in range(n_caption_step):
                with tf.device("/cpu:0"):
                    current_embed_sentence = tf.nn.embedding_lookup(
                        self.Wemb, reverse_caption[:, i]
                    )
                with tf.variable_scope("Reverse_LSTM_sentence"):
                    (
                        reverse_output1_sentence,
                        reverse_state1_sentence,
                    ) = self.reverse_lstm_sentence_dropout(
                        current_embed_sentence, reverse_state1_sentence
                    )
                    if i > 0:
                        tf.get_variable_scope().reuse_variables()
            ...
            reverse_sequence_sentence_emb = tf.reverse_sequence(
                reverse_output1_sentence_list, seq_lengths=caption_length, seq_dim=0, batch_dim=1
            )

            sequence_sentence_emb = tf.concat(
                [ordinal_sequence_sentence_emb, reverse_sequence_sentence_emb], 2
            )
            sequence_sentence_emb_flat = tf.reshape(
                sequence_sentence_emb, [-1, self.dim_hidden * 2]
            )  # (nxb,2h)
            sequence_sentence_combine = tf.nn.relu(
                tf.nn.xw_plus_b(
                    sequence_sentence_emb_flat, self.w_fuse_sentence, self.b_fuse_sentence
                )
            )
            sequence_sentence_combine = tf.reshape(
                sequence_sentence_combine, [n_caption_step, self.batch_size, self.dim_hidden]
            )
            ...
            brcst_w = tf.tile(
                tf.expand_dims(self.embed_att_w, 0), [self.n_frame_step, 1, 1]
            )  # n x h x 1
            image_part = (
                tf.matmul(
                    sequence_video_combine,
                    tf.tile(tf.expand_dims(self.embed_att_Ua, 0), [self.n_frame_step, 1, 1]),
                )
                + self.embed_att_ba
            )  # n x b x h
            ...
            alphas = tf.tile(
                tf.expand_dims(tf.div(e_hat_exp, denomin), 2), [1, 1, self.dim_hidden]
            )  # n x b x h  # normalize to obtain alpha
            attention_list = tf.multiply(
                alphas, sequence_video_combine
            )  # n x b x h ; multiply is element x element
            attended_video_fts = tf.reduce_sum(
                attention_list, 0
            )  # b x h       #  visual feature by soft-attention weighted sum
            ...
            sentence_part = (
                tf.matmul(
                    sequence_sentence_combine,
                    tf.tile(tf.expand_dims(self.embed_att_U_qa, 0), [n_caption_step, 1, 1]),
                )
                + self.embed_att_b_qa
            )
            ...
            alphas_q = tf.tile(
                tf.expand_dims(tf.div(e_hat_exp_q, denomin_q), 2), [1, 1, self.dim_hidden]
            )
            ...
            if self.model_mode == "AF":
                ...
                multimodel_fts_hidden = tf.nn.relu(
                    tf.nn.xw_plus_b(multimodel_fts_concat, self.w_fuse, self.b_fuse)
                )
                if self.regress_layer_num == 1:
                    predict_location = tf.nn.relu(
                        tf.nn.xw_plus_b(multimodel_fts_hidden, self.regress_w, self.regress_b)
                    )  # b x 2
                else:
                    predict_hidden = tf.nn.xw_plus_b(
                        multimodel_fts_hidden, self.regress_w, self.regress_b
                    )  # b x h
                    predict_location = tf.nn.relu(
                        tf.nn.xw_plus_b(predict_hidden, self.regress_w_2, self.regress_b_2)
                    )  # b x 2
            else:  # AW
                if self.regress_layer_num == 1:
                    predict_location = tf.nn.relu(
                        tf.nn.xw_plus_b(predict_attention_weights_v, self.regress_w, self.regress_b)
                    )  # b x 2
                else:
                    predict_hidden = tf.nn.xw_plus_b(
                        predict_attention_weights_v, self.regress_w, self.regress_b
                    )  # b x h
                    predict_location = tf.nn.relu(
                        tf.nn.xw_plus_b(predict_hidden, self.regress_w_2, self.regress_b_2)
                    )  # b x 2
            ...
            loss_regression = tf.multiply(
                tf.constant(alpha_regress), tf.reduce_mean(loss_regression_tmp)
            )

            batch_ground_weight = tf.multiply(
                video_attention_weights, tf.log(predict_attention_weights_v + tf.constant(1e-12))
            )
            ...
            return (
                ...
                video_mask,
                video_location,
                video_attention_weights,
                caption,
                caption_mask,
                caption_emb_mask,
                loss_mask,
                predict_attention_weights_v,
                predict_attention_weights_q,
                reverse_caption,
                caption_length,
            )
...
def preProBuildWordVocab(
    logging, sentence_iterator, word_count_threshold=5
):  # borrowed this function from NeuralTalk
    logging.info(
        "preprocessing word counts and creating vocab based on word count threshold {:d}".format(
    ...
    for sent in sentence_iterator:
        ...
        for w in sent.lower().split(" "):
            ...
            )  # if w is not in word_counts, will insert {w:0} into the dict
...
def localizing_one(
    ...
    caption_emb_mask_tf,
    predict_location_tf,
    predict_attention_weights_v_tf,
    predict_attention_weights_q_tf,
):
    ...
    for ind in xrange(batch_size):
        ...
        if len(idx) == 0:
            continue
        current_video_mask[ind, : idx[-1] + 1] = 1.0
    ...
    reverse_current_caption_ind = map(
        lambda cap: [wordtoix[word] for word in cap.lower().split(" ")[::-1] if word in wordtoix],
        current_captions,
    )
    reverse_current_caption_matrix = sequence.pad_sequences(
        reverse_current_caption_ind, padding="post", maxlen=n_caption_step - 1
    )
    reverse_current_caption_matrix = np.hstack(
        [reverse_current_caption_matrix, np.zeros([len(reverse_current_caption_matrix), 1])]
    ).astype(int)
    ...
    predict_video_location, predict_attention_weight_v, predict_attention_weight_q = sess.run(
        [predict_location_tf, predict_attention_weights_v_tf, predict_attention_weights_q_tf],
        feed_dict={
            video_tf: current_video_feat,
...
def localizing_all(
    ...
    for _, video_feat_path in enumerate(val_data):
        ...
        predict_attention_weight_v_list = (
            predict_attention_weight_v_list + predict_attention_weights_v
        )
        predict_attention_weight_q_list = (
            predict_attention_weight_q_list + predict_attention_weights_q
        )
    ...
    np.savez(
        final_save_path,
        video=video_name_list,
        duration=duration_list,
        caption=caption_list,
        groundtruth_timestamps=grountruth_timestamps_list,
        predict_timestamps=predict_timestamps_list,
        predict_attention_weights_v=predict_attention_weight_v_list,
        predict_attention_weights_q=predict_attention_weight_q_list,
    )
...
def train(sub_dir, logging, regress_layer_num, model_save_dir, result_save_dir):
    ...
    meta_data, train_data, val_data = get_video_data_jukin(
        video_data_path_train, video_data_path_val
    )
    ...
    (
        ...
        tf_video_mask,
        tf_video_location,
        tf_video_attention_weights,
        tf_caption,
        tf_caption_mask,
        tf_caption_emb_mask,
        tf_loss_mask,
        tf_predict_attention_weights_v,
        tf_predict_attention_weights_q,
        tf_reverse_caption,
        tf_caption_length,
    ) = model.build_model()
    ...
    (
        ...
        test_video_mask_tf,
        test_caption_tf,
        test_caption_mask_tf,
        test_caption_emb_mask_tf,
        test_predict_location_tf,
        test_predict_attention_weights_v_tf,
        test_predict_attention_weights_q_tf,
        test_reverse_caption_tf,
        test_caption_length_tf,
        ...
        for current_batch_file_idx in xrange(len(train_data)):
            ...
            for ind in xrange(batch_size):
                ...
                if dataset == "ActivityNet":
                    ...
                    right = min(
                        n_frame_step, int(current_video_location[ind][1] * n_frame_step) + 1
                    )
                    current_weights[ind, left:right] = 1.0 / (right - left + 1)
                idx = np.where(current_batch["label"][:, ind] != -1)[
                    0
                ]  # find this video's segment finish point
                if len(idx) == 0:
                    continue
            ...
            reverse_current_caption_ind = map(
                lambda cap: [
                    wordtoix[word] for word in cap.lower().split(" ")[::-1] if word in wordtoix
                ],
                current_captions,
            )
            reverse_current_caption_matrix = sequence.pad_sequences(
            ...
            logging.info(
                "loss = {:f}  loss_regression = {:f}  loss_attention = {:f}".format(
                    loss_val, loss_regression, loss_attention
                )
            )
            tStop = time.time()
...
if __name__ == "__main__":
    ...
    sub_dir, logging, regress_layer_num, model_save_dir, result_save_dir = make_prepare_path(
        args.regress_layer_num, dataset
    )
