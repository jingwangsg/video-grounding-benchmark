...
class ActivityNetProcessor:
    ...
    def process_data(self, data, scope):
        ...
        for vid, data_item in tqdm(
            data.items(), total=len(data), desc="process activitynet {}".format(scope)
        ):
...
class TACoSProcessor:
    ...
    def process_data_tan(self, data, scope):
        results = []
        for vid, data_item in tqdm(
            data.items(), total=len(data), desc="process tacos {}".format(scope)
        ):
            if vid.endswith(".avi"):
                vid = vid[0:-4]
...
def gen_or_load_dataset(configs):
    ...
    if configs.suffix is None:
        save_path = os.path.join(
            configs.save_dir,
            "_".join([configs.task, configs.fv, str(configs.max_pos_len)]) + ".pkl",
        )
    else:
        save_path = os.path.join(
            configs.save_dir,
    ...
    train_set = dataset_gen(
        train_data, vfeat_lens, word_dict, char_dict, configs.max_pos_len, "train"
    )
    val_set = (
        None
        if val_data is None
        else dataset_gen(val_data, vfeat_lens, word_dict, char_dict, configs.max_pos_len, "val")
    )
