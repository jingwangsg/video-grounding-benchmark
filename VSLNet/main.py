...
# data parameters
parser.add_argument(
    "--save_dir", type=str, default="datasets", help="path to save processed dataset"
)
...
parser.add_argument(
    "--max_pos_len", type=int, default=128, help="maximal position sequence length allowed"
)
# model parameters
...
parser.add_argument(
    "--char_dim", type=int, default=50, help="character dimension, set to 100 for activitynet"
)
...
parser.add_argument(
    "--suffix", type=str, default=None, help="set to the last `_xxx` in ckpt repo to eval results"
)
...
train_loader = TrainLoader(
    dataset=dataset["train_set"], visual_features=visual_features, configs=configs
)
...
home_dir = os.path.join(
    configs.model_dir,
    "_".join(
        [configs.model_name, configs.task, configs.fv, str(configs.max_pos_len), configs.predictor]
    ),
)
...
if configs.mode.lower() == "train":
    ...
    save_json(
        vars(configs), os.path.join(model_dir, "configs.json"), sort_keys=True, save_pretty=True
    )
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            for epoch in range(configs.epochs):
                for data in tqdm(
                    ...
                    desc="Epoch %3d / 3%d" % (epoch + 1, configs.epochs),
                ):
                    # run the model
                    feed_dict = get_feed_dict(data, model, drop_rate=configs.drop_rate)
                    _, loss, h_loss, global_step = sess.run(
                        [model.train_op, model.loss, model.highlight_loss, model.global_step],
                        feed_dict=feed_dict,
                    )
                    if global_step % configs.period == 0:
                        write_tf_summary(
                            writer,
                            [("train/loss", loss), ("train/highlight_loss", h_loss)],
                            global_step,
                        )
                    # evaluate
                    if global_step % eval_period == 0 or global_step % num_train_batches == 0:
                        ...
                        if r1i7 > best_r1i7:
                            ...
                            filename = os.path.join(
                                model_dir, "{}_{}.ckpt".format(configs.model_name, global_step)
                            )
